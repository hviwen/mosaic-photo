...
**当前问题：**
- 用户导入多张图片后，系统会自动进行智能排版，对每张照片应用智能裁剪（保存在 `photo.crop` 或 `photo.layoutCrop` 中）
...
如遇问题，请查看以下日志文件：
- `.template/.log/.info.log` - 常规运行日志
- `.template/.log/.warning.log` - 警告信息
- 浏览器 Console - 搜索 `[LayoutDebug]` 和 `[FaceDebug]` 标签