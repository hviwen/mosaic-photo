...
const isSelectedInCropMode = computed(
  () =>
    ...
    store.cropModePhotoId === selectedPhoto.value?.id,
);

const faceBoxes = computed(() => {
  void detectionTick.value;
  const photo = selectedPhoto.value;
  if (!photo) return [];
  const detections = getSmartDetections(photo.id) ?? [];
  return detections.filter(d => d.kind === "face").map(d => d.box);
});

const isFaceDetecting = computed(() => {
  void detectionTick.value;
  const photo = selectedPhoto.value;
...
let disposeDetections: (() => void) | null = null;

onMounted(() => {
  disposeDetections = onSmartDetectionsChanged(() => {
    detectionTick.value++;
  ...
  if ("ResizeObserver" in window) {
    ...
    if (originPreviewCanvasEl.value) {
      originPreviewResizeObserver.value.observe(originPreviewCanvasEl.value);
    }
  }
  void nextTick(() => {
    scheduleOriginPreviewRender();
...
onUnmounted(() => {
  ...
  if (originPreviewRaf.value != null) {
    cancelAnimationFrame(originPreviewRaf.value);
    originPreviewRaf.value = null;
  }
});
  ...
  if (!Number.isNaN(value)) {
    if (!transformStart.value) {
      transformStart.value = {
        scale: selectedPhoto.value.scale,
        rotation: selectedPhoto.value.rotation,
      };
    }
...
function updateRotation(v: unknown) {
  ...
  if (!Number.isNaN(deg)) {
    ...
    store.updatePhoto(selectedPhoto.value.id, {
      rotation: degreesToRadians(deg),
    });
    scheduleTransformCommit();
  }
}
...
function confirmCrop() {
  window.dispatchEvent(new Event(CROP_CONFIRM_EVENT));
}
...
function updateAdjustments<K extends keyof PhotoAdjustments>(
  key: K,
  v: unknown,
) {
  if (!selectedPhoto.value) return;
...
watch(
  () => [
    selectedPhoto.value?.id,
    selectedPhoto.value?.imageWidth,
    selectedPhoto.value?.imageHeight,
    selectedPhoto.value?.srcUrl,
    detectionTick.value,
...
function scheduleOriginPreviewRender() {
  if (originPreviewRaf.value != null) return;
  originPreviewRaf.value = window.requestAnimationFrame(() => {
    originPreviewRaf.value = null;
    drawOriginPreview();
  });
}

function drawOriginPreview() {
  const canvas = originPreviewCanvasEl.value;
  if (!canvas) return;
  const photo = selectedPhoto.value;
  ...
  const imageW = Math.max(1, photo.imageWidth);
  const imageH = Math.max(1, photo.imageHeight);
