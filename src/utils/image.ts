...
export async function createPhotoFromFile(
  ...
  try {
    const maxImageEdge = Math.max(
      256,
      Math.round(options?.maxImageEdge ?? MAX_IMAGE_EDGE),
    );
...
async function decodeImageFile(file: File): Promise<DecodedImageSource> {
  ...
  if (typeof createImageBitmap === "function") {
    try {
      ...
      let srcUrl: string;
      try {
        srcUrl = await createPreviewUrlFromSource(
          bitmap,
          bitmap.width,
          bitmap.height,
        );
...
function resolveImageSize(img: CanvasImageSource): {
  width: number;
  height: number;
} {
  if (
...
export function centerCropToAspect(
  ...
  if (
    !isFinite(currentAspect) ||
    Math.abs(currentAspect - targetAspect) < 1e-6
  ) {
    return crop;
  }
